...
NOTE: Communication with server procedure.
      ...
      Disconnect:
        1 Send id of who contacts: "raoches"
...
int roaches_initialize(void **requester, ConnectRepply** bots, int argc, char*argv[]){
    ...
    *requester = zmq_socket (context, ZMQ_REQ);
    ...
    if(check == -1){
        ...
        // Receive message with ids and tokens
...
void bot_input(void* requester, ConnectRepply* roaches,int number_bots){
    ...
    // Variable to send movement messages initialization 
   ...
   while(1){ 
        ...
        // If control_c was pressed finish cycle
        if(ctrl_c_pressed==1){
